...
t1 = toc;
title(['COP kmeans ',sprintf('%.2f s',t1)])
tic
constrained_hierarchical(data,2, c_join, c_repel, [], 1);
t2 = toc;
title(['Constrained single linkage ',sprintf('%.2f s',t2)])
drawnow
...
tic
assigned_labels = constrained_hierarchical(data,2, c_join, c_repel, [], 0);
plot_3D_clusters(data,assigned_labels, 2, c_join, c_repel)
rotate3d
view([23 17])
t2 = toc;
